...
import { FinishReporter, Service, Job } from '../../.'
...
export async function onFinish(p: FinishParams): Promise<void> {
  ...
  if (!p.config.posturl) {
    error('finishReporters#slack: Slack requires a posturl and you did not pass one');
    return;
  }

  
  await tiny.post({ url: p.config.posturl, data: message, headers: { 'content-type': 'application/json' } })
  .then(() => info('Successfully posted message to slack about job with status ', p.status))
  .catch((e: any) => {
